...
filepath = "dve-coarse.256-model-{epoch:02d}-{loss:.2f}.hdf5"
...
x_train *= int(1.0*latent_dim/x_train.max())
x_test *= int(1.0*latent_dim/x_test.max())
...
vae.save_weights('vae256._mlp_mnist_latent_dim_%s.h5' %latent_dim)
encoder.save('encoder.fine.256.h5')
