...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test,preProcTrain,preProcTest):
    ...
    X_coarseTrain = np.array([x.reshape( (32,32, 1) ) for x in coarseTrain])
    X_coarseTest = np.array([x.reshape( (32,32, 1 ) ) for x in coarseTest])
   ...
   #seqa=SeqSelfAttention(attention_activation='sigmoid')(ls11a)
    ...
    # 98x98x24    
    #'''
    pool_2b = MaxPooling2D((2,2), strides=(2,2))(conv_1b)
    #print(pool_2b.shape)
    ...
    re_4b = Reshape(target_shape=(8, 8),input_shape=(8, 8,1))(act_3b)
    #coarseEncoder.end()
...
if __name__ == '__main__':
    ...
    coarseEncoder = load_model('encoder_latent_dim_1024.h5')#, custom_objects={'Sampling': Sampling}, compile =False)
