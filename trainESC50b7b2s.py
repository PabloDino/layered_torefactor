...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test):
    ...
    X_fineTrain = np.array([x.reshape( (128, 128, 1) ) for x in fineTrain])
    X_fineTest = np.array([x.reshape( (128, 128, 1) ) for x in fineTest])
  

    X_coarseTrain = np.array([x.reshape( (128, 128, 1) ) for x in coarseTrain])
    X_coarseTest = np.array([x.reshape( (128, 128, 1) ) for x in coarseTest])
    ...
    l_input_shape_a=(128, 128, 1)
    input_shape_a=(128, 128, 1)
    model_a_in = Input(shape=input_shape_a)
    re_1a = Reshape(target_shape=(128, 128),input_shape=(128, 128, 1))(model_a_in)
 
    l1a = layers.LSTM(input_dim//latent_dim, return_sequences=True)(re_1a)    
    b1a = layers.Bidirectional(layers.LSTM(input_dim//latent_dim//2, return_sequences=True))(l1a)
    re_2a = Reshape(target_shape=(16,128,1),input_shape=(16,128))(b1a)
    '''
