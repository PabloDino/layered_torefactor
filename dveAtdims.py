...
input_shape_b=(4*dataSize*dataSize)
input_shape_v=(200)
time_steps =64
#######################################################################
...
print('q-val-attn-seq', query_value_attention_seq.shape)

re_3b = layers.Reshape(target_shape=(256, 256,1),input_shape=(None, 4, 128, 128, 1))(query_value_attention_seq)
print('re_3b', re_3b.shape)

pool_3b = layers.MaxPooling2D((2,2), strides=(2,2))(re_3b)
print('pool_3b',pool_3b.shape)

re_3b2 = layers.Reshape(target_shape=(256,256),input_shape=(4, 128, 128, 1))(query_value_attention_seq)
print('re_3b', re_3b.shape)
...
#print('input shape is ', input_layer.shape)
model = tf.keras.Model(inputs=[query_input, value_input],
                       outputs=[query_embeddings, attention_output])
names = ('query_embeddings', 'attention_output')
...
re_3ba = layers.Reshape(target_shape=(8,64),input_shape=(None, 512))(input_layer)
...
#print(ls_5b.shape)
re_5b = layers.Reshape(target_shape=(16,32,1),input_shape=(16,32))(re_3ba)
...
#vae.fit([[query_input,value_input],decoder_outputs], epochs=epochs, batch_size=32, callbacks=[early_stopping_monitor]) #,validation_data=(x_test, None))
np.pad(y_train,65536)
vae.fit([np.expand_dims(x_train,1),np.expand_dims(y_train,1)], epochs=epochs, batch_size=32, callbacks=[early_stopping_monitor]) #,validation_data=(x_test, None))
