...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test,preProcTrain,preProcTest):
    ...
    print("trainCoarse shape is ", coarseTrain[0].shape)
    print("trainFine shape is ", fineTrain[0].shape)
    ...
    X_fineTrain = np.array([x.reshape( (16,16, 1) ) for x in fineTrain[0]])
    X_fineTest = np.array([x.reshape( (16,16, 1 ) ) for x in fineTest[0]])
  

    X_coarseTrain = np.array([x.reshape( (4, 4, 1) ) for x in coarseTrain[0]])
    X_coarseTest = np.array([x.reshape( (4,4, 1 ) ) for x in coarseTest[0]])
