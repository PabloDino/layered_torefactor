...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test,preProcTrain,preProcTest):
    ...
    X_coarseTrain = np.array([x.reshape( (8, 4, 1) ) for x in coarseTrain])
    X_coarseTest = np.array([x.reshape( (8,4, 1 ) ) for x in coarseTest])
    ...
    l_input_shape_b=(8,4 ,1,1)
    input_shape_b=(8,4,1)

    model_b_in = Input(shape=input_shape_b)
    print(model_b_in.shape)

    conv_1b = Conv2D(1, (1,1), strides=(2, 1), input_shape=input_shape_a)(model_b_in)#placeholder
...
if __name__ == '__main__':
    ...
    coarseEncoder = load_model('encoder_latent_dim_16.h5')
    ...
    for i in range(0,int((numrows/viewBatch))):#print(x_train.shape)
      ...
      fineBatch, _ = fineEncoder.predict(sample)
      if (i==0):
        fineCoded=fineBatch
      else:
    ...
    for i in range(0,int((numrows/viewBatch))):#print(x_train.shape)
      ...
      coarseBatch, _ = coarseEncoder.predict(sample2)
      if (i==0):
        coarseCoded=coarseBatch
      else:
        coarseCoded = np.concatenate((coarseCoded,coarseBatch), axis=0)
      print('coding train coarse', coarseCoded.shape)
    ...
    for i in range(0,int((numrows/viewBatch))):#print(x_train.shape)
      ...
      fineBatch, _ = fineEncoder.predict(sample3)
      if (i==0):
        fineCoded=fineBatch
      else:
        fineCoded = np.concatenate((fineCoded,fineBatch), axis=0)
    ...
    for i in range(0,int((numrows/viewBatch))):#print(x_train.shape)
      ...
      coarseBatch, _ = coarseEncoder.predict(sample4)
      if (i==0):
        coarseCoded=coarseBatch
      else:
        coarseCoded = np.concatenate((coarseCoded,coarseBatch), axis=0)
      print('coding test coarse', coarseCoded.shape)
    ...
    ###########################################
    #X_train_coarse_encoded = np.reshape(X_train_coarse_encoded,(X_train_coarse_encoded.shape[1],X_train_coarse_encoded.shape[2],X_train_coarse_encoded.shape[0],X_train_coarse_encoded.shape[3]))
    #X_train_coarse_encoded = np.reshape(X_train_coarse_encoded,(X_train_coarse_encoded.shape[1],X_train_coarse_encoded.shape[2], X_train_coarse_encoded.shape[0],X_train_coarse_encoded.shape[3]))
