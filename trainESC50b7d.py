...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test,preProcTrain,preProcTest):
    ...
    X_fineTrain = np.array([x.reshape( (640, 640, 1) ) for x in fineTrain])
    X_fineTest = np.array([x.reshape( (320, 320, 1 ) ) for x in fineTest])
  

    X_coarseTrain = np.array([x.reshape( (160, 320, 1) ) for x in coarseTrain])
    X_coarseTest = np.array([x.reshape( (80,160, 1 ) ) for x in coarseTest])
...
if __name__ == '__main__':
    ...
    fineEncoder = load_model('encoder.fine.h5'  , custom_objects={'Sampling': Sampling}, compile=False)
    coarseEncoder = load_model('encoder.coarse.h5', custom_objects={'Sampling': Sampling}, compile =False)
