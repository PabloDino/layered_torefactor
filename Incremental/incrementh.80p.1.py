...
def encPredict(enc, x_train):
   ...
   for i in range(0,int((numrows/viewBatch))):#print(x_train.shape)
      ...
      #if True:#(i==0):
      #   print('INPUT:::',sample.shape,'OUTPUT:::',z_mean8[0].shape)
   ...
   print(len(z_mean) ,' things ::', z_mean[0][0].shape)
...
def fitCombined(X_train50p1_encoded, X_train30p1_encoded, X_train50p5_encoded, X_train30p5_encoded, X_test50p1_encoded, X_test30p1_encoded, X_test50p5_encoded, X_test30p5_encoded, y_train, y_test,combModel, layernum1, layernum2,perc):#,fixedLayers):
    ...
    combModel.fit(indata,
        ...
        validation_data= ([X_test50p1_encoded, X_test30p1_encoded, X_test50p5_encoded, X_test30p5_encoded,], y_test),#,
    ...
    score = combModel.evaluate([X_test50p1_encoded, X_test30p1_encoded, X_test50p5_encoded, X_test30p5_encoded],
        y=y_test)
...
def mergeModels(mod50p10, mod30p10,mod50p50, mod30p50, layernum1, layernum2):
    ...
    mod50pend10 = mod50p10.layers[-1].get_output_at(0)
    mod30pstart10 = mod30p10.layers[0].get_output_at(0)
    mod30pend10 = mod30p10.layers[-1].get_output_at(0)
    
    mod50pstart50 = mod50p50.layers[0].get_output_at(0)
    mod50pend50 = mod50p50.layers[-1].get_output_at(0)
    mod30pstart50 = mod30p50.layers[0].get_output_at(0)
    mod30pend50 = mod30p50.layers[-1].get_output_at(0)
    ...
    #print("layernums = ", layernum1, layernum2,"mod50pend1 shape is ", mod50pend1.shape)
 
    if (layernum1>20):
        in50p1 = Input((64,))
        in30p1 = Input(( 64,))
    else:
        in50p1 = Input(K.int_shape(mod50pend1))
        in30p1 = Input(K.int_shape(mod30pend1))
        #in50p1 = Input(mod50pend1.shape)
        #in30p1 = Input(mod30pend1.shape)
    ...
    if (layernum1<=20) and (layernum2<=127):
     for i in range (layernum1+1,len(mod50p1.layers)-1):
      layer = mod50p1.layers[i]
     ...
     drop50p1=Dropout(0.5)(nextLyr)
     numdims=1
     #print('drop50p1 shape is ', drop50p1.shape)

     for i in range(len(K.int_shape(drop50p1))):
        #print('i=',i,'numdims=',numdims)
        if K.int_shape(drop50p1)[i]!=None:
           numdims=numdims*K.int_shape(drop50p1)[i]
           
     re50p1 =Reshape((numdims,))(drop50p1)       
     dense50p1 = Dense(units=64, name ='denseout50p1')(re50p1)       
     #mod50p1 = Model(in50p1,nextLyr)
     #print('50p1: numdims=',numdims, drop50p1.shape,re50p1.shape,dense50p1.shape  )
     #print(' summary', )
     #mod50p1.summary()

     nextLyr = in30p1
     for i in range (layernum1+1,len(mod30p1.layers)-1):
        layer = mod30p1.layers[i]
        #if (currlyr >= startLayer):
        if isinstance(layer, keras.layers.Conv2D):
            nextLyr = Conv2D(filters=layer.filters,kernel_size=layer.kernel_size, padding=layer.padding, strides=layer.strides,  kernel_regularizer=layer.kernel_regularizer)(nextLyr)#, layer.get_output_at(0).shape
        if isinstance(layer, keras.layers.MaxPooling2D):
        ...
        if isinstance(layer, keras.layers.AveragePooling2D):
           nextLyr = AveragePooling2D(pool_size=layer.pool_size, strides=layer.strides)(nextLyr)
     drop30p1=Dropout(0.5)(nextLyr)
     numdims=1
     for i in range(len(K.int_shape(drop30p1))):
        #print('i=',i,'numdims=',numdims)
        if K.int_shape(drop30p1)[i]!=None:
           numdims=numdims*K.int_shape(drop30p1)[i]
     #print('numdims=',numdims)
     re30p1 = Dense(units=numdims, name ='denseout30p1')(drop50p1)       
     ...
     nextLyr = in50p5
     for i in range (layernum2+1,len(mod50p5.layers)-1):
      layer = mod50p5.layers[i]
      #if (currlyr >= startLayer):
      if True:#currlyr <(numlyrs-2):
        if isinstance(layer, keras.layers.Conv2D):
            nextLyr = Conv2D(filters=layer.filters,kernel_size=layer.kernel_size, padding=layer.padding, strides=layer.strides,  kernel_regularizer=layer.kernel_regularizer)(nextLyr)#, layer.get_output_at(0).shape
     ...
     drop50p5=Dropout(0.5)(nextLyr)
     numdims=1
     for i in range(len(K.int_shape(drop50p5))):
        #print('i=',i,'numdims=',numdims)
        if K.int_shape(drop50p5)[i]!=None:
           numdims=numdims*K.int_shape(drop50p5)[i]
     #print('numdims=',numdims)
           
     re50p5 = Reshape((numdims,))(drop50p5)       
     ...
     nextLyr = in30p5
     for i in range (layernum2+1,len(mod30p5.layers)-1):
      layer = mod30p5.layers[i]
      #if (currlyr >= startLayer):
      if True:#currlyr <(numlyrs-2):
        if isinstance(layer, keras.layers.Conv2D):
            nextLyr = Conv2D(filters=layer.filters,kernel_size=layer.kernel_size, padding=layer.padding, stri0des=layer.strides,  kernel_regularizer=layer.kernel_regularizer)(nextLyr)#, layer.get_output_at(0).shape
        if isinstance(layer, keras.layers.MaxPooling2D):
           nextLyr = MaxPooling2D(pool_size=layer.pool_size)(nextLyr)
        if isinstance(layer, keras.layers.Concatenate):
           pos0 =findLayer(lyrs, layer.get_input_at(0)[0])
        ...
        if isinstance(layer, keras.layers.AveragePooling2D):
           nextLyr = AveragePooling2D(pool_size=layer.pool_size, strides=layer.strides)(nextLyr)
     drop30p5=Dropout(0.5)(nextLyr)
     #print('nextLyr shape:', nextLyr.shape)
     numdims=1
     for i in range(len(K.int_shape(drop30p5))):
        #print('i=',i,'numdims=',numdims)
        if K.int_shape(drop30p5)[i]!=None:
           numdims=numdims*K.int_shape(drop30p5)[i]
     #print('numdims=',numdims)
           
     re30p5=Reshape((numdims,))(drop30p5)
    ...
    if (layernum1>20) and (layernum2>127):
       ...
       #print("in50p5 shape is ", in50p5.shape)
       #print("in30p1 shape is ", in30p5.shape)  
       concatC = Concatenate(name='outconCat',axis=1) ([in50p1,in30p1,in50p5,in30p5])
    else:
       #print("dense50p1 shape is ", dense50p1.shape)
       #print("dense30p1 shape is ", dense30p1.shape)         
       #print("dense50p5 shape is ", dense50p5.shape)
       #print("dense30p5 shape is ", dense30p5.shape)    
       concatC = Concatenate(name='outconCat',axis=1) ([dense50p1,dense30p1,dense50p5,dense30p5])
    ...
    newModel = Model([in50p1,in30p1,in50p5,in30p5], prediction)
...
if __name__ == '__main__':
    ...
    c50p1Count = len(mod50p1.layers)
    c30p1Count = len(mod30p1.layers)
    c50p5Count = len(mod50p5.layers)
    c30p5Count = len(mod30p5.layers)
    ...
    for perc0 in range(3,100):
     ...
     if ((not(prevc50p1Lev==c50p1Lev)) or (not init)):#codifying data for mod50p1
      ...
      if (c50p1Count==c50p1Lev):
         ...
         encout = encPreModel.layers[-1].get_input_at(0)
      
      encout = encPreModel.layers[-2].get_input_at(0)
      print('encout.shape is ', encout.shape, ', name is ',encout.name )
      #''' 
     ...
     combModel = mergeModels(enc50p1, enc30p1,enc50p5, enc30p5, c50p1Lev, c50p5Lev)
     #combModel.summary()
     try:
     #if True:#
        fitCombined(X_train50p1_encoded, X_train30p1_encoded, X_train50p5_encoded, X_train30p5_encoded, X_test50p1_encoded, X_test30p1_encoded, X_test50p5_encoded, X_test30p5_encoded, y_traincat, y_testcat, combModel, c50p1Lev, c50p5Lev,perc)
     except:
