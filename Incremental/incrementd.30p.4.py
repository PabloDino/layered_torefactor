...
filepath = 'Model.2.{epoch:02d}-{loss:.2f}.hdf5'
...
def fitCombined(xTrain,xTest, y_train, y_test,combModel,fixedLayers):
    ...
    combModel.save('Inc.4.'+str(fixedLayers)+'_lyrs.'+str(round(score[1],3))+'.'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+'.hdf5')
    #'''
...
if __name__ == '__main__':
    ...
    availCount= int(totalRecordCount*0.1)
    ...
    #print(os.getcwd())
    modelbase =keras.models.load_model('50p/5b/Model.4.final.hdf5')#, custom_objects={'tf': tf}, compile =False)
    
    #cloned = keras.models.clone_model(modelbase)
    #modelbase.summary()
 
    #################################################################################
    
    orig_in = modelbase.layers[0].get_output_at(0)
    #cloned.build(orig_in.shape)
    '''
   ...
   ''' 
    ...
    #smodelbase._layers.pop()
    modelbase._layers.pop()
    modelout = modelbase.layers[-1].get_output_at(0)
    
    preDense=Dropout(0.8)(modelout)
    denseOut=Dense(totalLabel,name='denseout')(preDense)
    modelbase.summary()
    ...
    prediction = Dense(totalLabel, activation='softmax')(denseOut)
    
        
    #model_c= Model(inputs=model_a_in, outputs=act_13a, name='comp')
    #
    #for i in range(len(modelbase.layers[:-1])):
    i= len(modelbase.layers[:-1])
    if True:
      modelFixed=Model(inputs=orig_in,outputs=prediction)
      #for j in range(len(modelFixed.layers)):
      #  init_layer(modelFixed.layers[j])
      print('outer lyer ', i, ' init all layers')
      for j in range(i):
        modelFixed.layers[j].set_weights(modelbase.layers[j].get_weights())
        modelFixed.layers[j].trainable=False
        print('@', j, 'fixed weights for ', modelFixed.layers[j])
      #try:
      fitCombined(x_train, x_test,  y_traincat, y_testcat, modelFixed, i)  
      #except:
      #   print("error fitting model with ",i, " frozen layers")
