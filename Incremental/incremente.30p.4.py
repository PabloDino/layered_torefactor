...
def fitCombined(xTrain,xTest, y_train, y_test,combModel,fixedLayers):
    ...
    combModel.save('Inc.branch.4.'+str(fixedLayers)+'_lyrs.'+str(round(score[1],3))+'.'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+'.hdf5')
    #'''
...
if __name__ == '__main__':
    ...
    #print(os.getcwd())
    modelbase =keras.models.load_model('50p/5b/Model.4.final.hdf5')#,  custom_objects={'tf': tf})
    modelbase.summary()
    #################################################################################
