...
def fitCombined(xTrain,xTest, y_train, y_test,combModel,fixedLayers):
    ...
    loss, acc,f1,precision, recall  = combModel.evaluate(xTest, y=y_test)

    print('Test loss:', loss)
    print('Test accuracy:', acc)

    if os.path.exists("densepipe.drop.perf.txt"):
       append_write = 'a' # append if already exists
    else:
       append_write = 'w' # make a new file if not

    outfile=open("densepipe.perf.txt","a")#append_write)
    outfile.write(str(fixedLayers)+","+ str(loss)+","+ str(acc) +","+str(f1)+","+str(precision)+","+str(recall)+","+ str(datetime.datetime.now())+"\n" )
    outfile.close()
...
if __name__ == '__main__':
    ...
    for i in range(6, len(modelbase.layers[:-1]),10):
