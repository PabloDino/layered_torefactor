...
def fitCombined(xTrain,xTest, y_train, y_test,combModel,fixedLayers):
    ...
    #combModel.save('Inc.branch.1.'+str(fixedLayers)+'_lyrs.'+str(round(score[1],3))+'.'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+'.hdf5')
    #'''
...
def cloneBranchedModel(modelbase, startLayer,totalLabel, origBranch):
    ...
    #   startLayer=10
    inLyr=origBranch.layers[0].get_output_at(0)
    branchIn=None
    if len(input_shape_a)==4:
      ...
      if len(input_shape_a)==3:
        ...
        if startLayer>=14 and startLayer <16:
           #inLyr = Input((None,4096))
           branchIn = Input((None,4096))
        else:#startLayer==16
           #inLyr=Input((None,64))
           branchIn=Input((None,64))
    ...
    nextBr = Reshape(modelbase.layers[startLayer].get_output_at(0).shape)(nextBr)
    for layer in modelbase.layers:
     if (currlyr >= startLayer):
      print(currlyr,':layer name is ', layer.name, ' ; ', nextLyr.name)
      if not isinstance(nextBr, keras.layers.Reshape):
          print(nextBr.shape, '==>',str(layer.name), layer.get_output_at(0).shape)
     ...
     currlyr+=1 
    #V2179842T3
    nextLyr=origBranch.layers[-1].get_output_at(0)
...
if __name__ == '__main__':
    ...
    for i in range(2, len(modelbase.layers[:-2])):
      ...
      modelNew = cloneBranchedModel(modelbase, startLyr, totalLabel, otherBranch)
      #modelNew.summary()
