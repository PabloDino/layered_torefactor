...
def fitCombined(xTrain,xTest, y_train, y_test,combModel,fixedLayers):
    ...
    combModel.save('Inc.3.'+str(fixedLayers)+'_lyrs.'+str(round(score[1],3))+'.'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+'.hdf5')
    #'''
...
if __name__ == '__main__':
    ...
    #print(os.getcwd())
    modelbase =keras.models.load_model('50p/3/Model.3.final.hdf5')#, custom_objects={'sampling': sampling}, compile =False)
    modelbase.summary()
    #################################################################################
   ...
   ''' 
    ...
    for i in range(1, len(modelbase.layers[:-2])):
      modelFixed=Model(inputs=orig_in,outputs=out)
