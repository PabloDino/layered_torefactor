...
def buildModel(fineTrain,coarseTrain,fineTest,coarseTest, y_train, y_test,preProcTrain,preProcTest):
    ...
    X_coarseTrain = np.array([x.reshape( (4,8, 1) ) for x in coarseTrain])
    X_coarseTest = np.array([x.reshape( (4,8, 1 ) ) for x in coarseTest])
    ...
    conv_1a = Conv2D(1, (1, 1), strides=(1, 1), input_shape=input_shape_a)(model_a_in)
    ...
    '''
    pool_6a=MaxPooling2D((2,2), strides=(3,3))(conv_1a)
    act_7a = Activation('relu')(pool_6a)
    print('act_7a', act_7a.shape)
    conv_7a = Conv2D(1, (2,2), strides=(1, 1), input_shape=input_shape_a)(act_7a)
 
    #re_10a = Reshape(target_shape=(4,4),input_shape=(2,24,48))(act_9a)
    re_10a = Reshape(target_shape=(4,4),input_shape=(4,4,1))(conv_7a)
    ...
    l_input_shape_b=(4,8 ,1,1)
    input_shape_b=(4,8,1)

    model_b_in = Input(shape=input_shape_b)
    print(model_b_in.shape)

    conv_1b = Conv2D(1, (1,1), strides=(1,1), input_shape=input_shape_a)(model_b_in)#placeholder
    print('conv1b', conv_1b.shape)
    # Using CNN to build model
    # 24 depths 128 - 5 + 1 = 124 x 124 x 24   
    # 98x98x24    
   
    pool_2b = MaxPooling2D((1,1), strides=(1,2))(conv_1b)
    print(pool_2b.shape)
    '''
    conv_3b = Conv2D(48, (3,3), strides=(1, 1), input_shape=input_shape_a)(pool_2b)
    print(conv_3b.shape)
     '''
    act_3b =Activation('relu')(pool_2b)
    print(act_3b.shape)
   

    re_4b = Reshape(target_shape=(4,4),input_shape=(4,4,1))(act_3b)
    #coarseEncoder.end()
    #ls_5b= LSTM(4,return_sequences=True,unit_forget_bias=1.0,dropout=0.1)(re_4b)
    #ls_5b   = layers.Bidirectional(layers.LSTM(32, return_sequences=True))(re_4b)

    #seqb=SeqSelfAttention(attention_activation='sigmoid')(ls_5b)
    #merge
 
   
    merged = Concatenate(axis=1)([re_10a,re_4b])
    #merged = Concatenate(axis=1)([re_10a,re_10a])
...
if __name__ == '__main__':
    ...
    fineEncoder = load_model('encoder64.hdf5')
    #fineEncoder = load_model('encoder_latent_dim_256.h5')
    #)#   , custom_objects={'Sampling': Sampling}, compile=False)
    coarseEncoder = load_model('encoder64.hdf5')#, custom_objects={'Sampling': Sampling}, compile =False)
    ...
    #preProcTrain = np.expand_dims(x_train, -1).astype("float32") / np.max(x_train)
    #preProcTest = np.expand_dims(x_test, -1).astype("float32") / np.max(x_test)
    preProcTrain = np.array(x_train).astype("float32") / np.max(x_train)
    preProcTest = np.array(x_test).astype("float32") / np.max(x_test)
    print(preProcTrain.shape)
    ...
    X_train_fine_encoded = fineEncoder.predict(preProcTrain)
    X_train_coarse_encoded = coarseEncoder.predict(preProcTrain)
    
    X_test_fine_encoded = fineEncoder.predict(preProcTest)
    X_test_coarse_encoded = coarseEncoder.predict(preProcTest)
    ...
    buildModel(X_train_fine_encoded,X_train_coarse_encoded, X_test_fine_encoded,X_test_coarse_encoded, y_traincat, y_testcat,preProcTrain,preProcTest)
