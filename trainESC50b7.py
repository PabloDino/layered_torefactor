...
input_mbed_shape=(dataSize,dataSize,1,latent_fine_dim)
#######################################################################
...
if __name__ == '__main__':
    ...
    #fineEncoder.load_weights('fine.weights.hdf5')
    #coarseEncoder.load_weights('coarse.weights.hdf5')
    ...
    fine_train = int(1.0*latent_fine_dim/preProcTrain.max())
    fine_test = int(1.0*latent_fine_dim/preProcTest.max())

    coarse_train = int(1.0*latent_coarse_dim/preProcTrain.max())
    coarse_test = int(1.0*latent_coarse_dim/preProcTest.max())

        

    X_train_fine_encoded= fineEncoder.predict([[fine_train,fine_train]])  
    X_train_coarse_encoded= coarseEncoder.predict(coarse_train)
    X_test_fine_encoded= fineEncoder.predict(fine_test)  
    X_test_coarse_encoded= coarseEncoder.predict(coarse_test)
