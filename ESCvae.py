from keras.layers import Lambda, Input, Dense,Activation, Dropout, Conv2D,\
...
if __name__ == '__main__':
    ...
    ################
    input_shape_b=(image_size[0] ,image_size[1],1)

    model_b_in = Input(shape=input_shape_b)
    print(model_b_in.shape)

    conv_1b = Conv2D(1, (7,7), strides=(1, 1), input_shape=input_shape_b)(model_b_in)
    ...
    print(pool_2b.shape)
    conv_3b = Conv2D(1, (3,3), strides=(1, 1), input_shape=input_shape_b)(pool_2b)
    print(conv_3b.shape)
  
    act_3b =Activation('relu')(conv_3b)
    re_4b = Reshape(target_shape=(28*28,),input_shape=(1,28,28))(act_3b)
    preProc = Model(inputs=model_b_in,outputs=re_4b) 

    encoder_inputs = keras.layers.concatenate([model_b_in, re_4b])
    encoder, z_mean, z_log_var = encoder_model(encoder_inputs)
    decoder = decoder_model()
    # instantiate VAE model
    outputs = decoder(encoder(encoder_inputs)[2])
    vae = Model(outputs,inputs=model_b_in)

    reconstruction_loss = mse(model_b_in, outputs)
    ...
    #vae.build()
    #print(vae.summary())
    vae.fit(x_train, epochs=epochs, batch_size=batch_size, validation_data=(x_test, None),   callbacks=[early_stopping_monitor])
